...
class HomeRepository(
    ...
    suspend fun getLatestTransactions(preferredCurrency: String): List<TransactionItem> {
        ...
        suspend fun getAllTransactions(preferredCurrency: String, userId: Int): List<TransactionItem> {
            val incomes = incomeDao.getAllIncomes(userId).map { income ->
                TransactionItem.IncomeItem(
                    income = income.copy(
                        amount = CurrencyConverter.convertAmount(
                            income.amount,
            ...
            val expenses = expensesDao.getAllExpenses(userId).map { expense ->
                TransactionItem.ExpenseItem(
                    expense = expense.copy(
                        amount = CurrencyConverter.convertAmount(
                            expense.amount,
    ...
    /**
     * Fetches and converts a list of latest expenses to the user's preferred currency.
     * @param limit The number of latest expenses to fetch.
     * @param preferredCurrency The user's preferred currency.
     * @return A list of converted Expenses objects.
     */
